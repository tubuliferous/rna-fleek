...
"""PyInstaller spec for RNA-FLEEK macOS .app"""
...
exe = EXE(pyz, a.scripts, [], exclude_binaries=True,
          ...
          console=False,  # No console on Mac — output goes to Console.app
          icon='assets/icon.icns' if Path('assets/icon.icns').exists() else None)

coll = COLLECT(exe, a.binaries, a.zipfiles, a.datas,
               strip=False, upx=False, name='RNA-FLEEK')

app = BUNDLE(coll, name='RNA-FLEEK.app',
             icon='assets/icon.icns' if Path('assets/icon.icns').exists() else None,
             bundle_identifier='com.tubuliferous.rna-fleek',
             info_plist={
                 'CFBundleShortVersionString': '0.1.0',
                 'CFBundleName': 'RNA-FLEEK',
